...
int main()
{
	...
	for (int i = 0; i < number_of_images; i++)
	{
		if (rank == 0)
		{
			...
			for (int x = 0; x < size_of_each_rank_take; x++) {
				for (int k = 0; k < number_of_images; k++) {
					...
					avg += images_arr[k][x];//get average of each pixel
			...
			if (i == number_of_images - 1) 
			{
				...
				{
					MPI_Status status;
					//each rank send peace that work on it 
		...
		else if (rank <= size)
		{
			...
			if (i == number_of_images - 1)
			{
				int* local_threshold = new int[size_of_each_rank_take];
				MPI_Recv(local_threshold, size_of_each_rank_take, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);



				int* local_forground = new int[size_of_each_rank_take];
				int* localbackgroundimage = new int[size_of_each_rank_take];
				int avg = 0;
				for (int x = 0; x < size_of_each_rank_take; x++) {
					for (int k = 0; k < number_of_images; k++) {

						avg += array_of_images_array[k][x];

					}
					int difference;
					avg /= number_of_images;
					localbackgroundimage[x] = avg;
					difference = abs(localbackgroundimage[x] - local_threshold[x]);
					...
					{
						local_forground[x] = difference;
					}


				}
				MPI_Send(localbackgroundimage, size_of_each_rank_take, MPI_INT, 0, 0, MPI_COMM_WORLD);
				MPI_Send(local_forground, size_of_each_rank_take, MPI_INT, 0, 0, MPI_COMM_WORLD);
			}
		}
